...
var basePath1 = "http://172.16.12.223";//公司
var basePath2 = "http://192.168.0.103";//家里
var basePath3 = "http://v6.shengjing88.com";//生产/
...
mui.plusReady(function () {
    ...
    loginUser = plus.storage.getItem('loginUser')
    if (!device_uuid) {
        var info = ''
        // 获取设备uuid
        plus.device.getInfo({
...
Common.resetBaseApi=function (url){
    ...
    if(url==="/app/auth/login"){
        ...
        baseApi = basePath1+url;
    }else if(url==='/app/user/v2/auth/create'){
        baseApi = basePath1+url;
